...
class Optimization():
    ...
    def __init__(
        ...
        self.n_obs = self.env.obs_space.shape
        self.n_actions = self.env.action_space.n

        self.policy_net = q_func(self.n_obs[-1], self.n_actions).to(self.device)
        self.target_net = q_func(self.n_obs[-1], self.n_actions).to(self.device)
    ...
    def dqn_learning(self):
        ...
        for i_episode in range(self.num_episodes):
            ...
            for c in count():
                ...
                observation, reward, terminate, info = self.env.step(action.item())
                # done = terminate
                ...
                if terminate:
                    ...
                    info["avg reward"] = final_reward

                    if final_reward > max_reward:
                        self.info_stack[f"episode_{i_episode}"] = info
                        max_reward = float(final_reward)
                        self.key_rewards.append(i_episode)
                        final_layout_image = self.env.layout_image

                    break

            if i_episode%50 == 0:
                print(f"episode {i_episode} completed | Max reward: {max_reward} at Ep. {self.key_rewards[-1]}")
        ...
        self.info_stack["time"] = elapsed_time
        print(f"Reward: {max(ep_rewards)}  |  Time: {elapsed_time} s")

        file_name = utils.export_results(self.info_stack, self.key_rewards)
        final_layout_image.save(f"./result/{file_name}_layout.jpg")
